...
function setJEAmountAndCSVImportAfterSubmit(type)
{
    ...
    try{
            ...
            if(arrTotal){
                ...
                if(!flTotalDebit){
                    ...
                    try{
                        ...
                        nlapiSubmitField(stTransRecordType, intRecordId, [FLD_TOTAL_DEBIT_AMT,FLD_TRANS_DEBIT_AMT], [Math.abs(total.toFixed(2)),Math.abs(fTotalAmt.toFixed(2))]);
                    }catch(error){
        ...
        if (type == 'create' && executionContext == 'csvimport'){
            ...
            if(intlineNum < 1 )
            	intlineNum= nlapiGetLineItemCount('item');
            if(intlineNum < 1 )
            	intlineNum= nlapiGetLineItemCount('expense');
            if( intlineNum > 400){
						
				var arrSchedParam = {};
				arrSchedParam['custscript_nsts_gawtrigrectype'] = stTransRecordType;
				arrSchedParam['custscript_nsts_gawtrigrecid'] = intRecordId;
				arrSchedParam['custscript_nsts_gawtrigwfid'] = stworkflow_id;
				arrSchedParam['custscript_nsts_gawtrigsubmit'] = WACT_GAW_SUBMIT_BTN;
				
				var status = nlapiScheduleScript('customscript_nsts_gaw_ss_trig_submit', null, arrSchedParam);
				nlapiLogExecution('debug','TRIGGER WORKFLOW SUBMIT','ON scheduled script');
			}else{
...
function validateTransSetOrderStatBeforeSubmit(type)
{
	...
	if(type =='create' && stTransRecordType != REC_RULE_GRP.toUpperCase()){
        ...
        nlapiSetFieldValue(FLD_SUPER_APPROVED, 'F', false, false);       	
      	...
      	if(executionContext != 'csvimport') {
			nlapiSetFieldValue('custbody_nsts_gaw_created_by', currentContext.getUser());
		}
    }
}