...
function GetEmailTemplate(bIsWithECP,type,objTransInfo,objLoadRecord,objLoadEmailTemplate){
    ...
    if(isEmpty(stEmailTemplateId)){
        HC_PENDING_APPROVAL_SUBJECT             = "{typeLabel} # {transactionnumber} is Pending Approval";
        HC_PENDING_APPROVAL_BODY_WITHECP        =   "<p>Hi {approverName},</p>"+
                                                    "<p>This {typeLabel} # {transactionnumber} is waiting for your approval.</p>"+
                                                    "<p>You may <a href='mailto:{ecpaddress}?subject=APPROVED:%20{type}%20No.%20{transactionnumber}"+
                                                    "&body=Reason:' target='_top'>approve bill1</a> or" +
                                                    "<a href='mailto:{ecpaddress}?subject=REJECTED:%20{type}%20No.%20{transactionnumber}"+
        ...
        HC_PENDING_APPROVAL_BODY_WITHOUTECP =  " <p>Hi {approverName},</p>"
                                                    ...
                                                    + "&tranId={id}&tranType={type}&approverRole={role}&action=1'>approve bill2</a> or "
 ...
 * @returns {String}
 */
function GetEmailTemplateForApproveReject(stSendOption,tranId){
    ...
    if(stSendOption == "approve"){
        stRetValue.subject  = "Transaction # {transactionnumber} is Approved";
        stRetValue.body     = 'Hi {creator},<br />\
                            <br />\
                            This transaction # {transactionnumber} is now approved.<br />\
        ...
        stRetValue.subject  = "Transaction # {transactionnumber} is rejected";
        stRetValue.body     = '<p>Hi {creator},</p>\
                            <p>This transaction # {transactionnumber} was rejected due to the following reasons:</p>\
