...
function hierarchyApprovalType(startIdx,recTrans,objDetails)
{
	...
	try{
	    ...
	    //Get approver limit
		try{		
			var objEmpRec = nlapiLoadRecord('employee',objDetails.idApprover);
			approverLimit = objEmpRec.getFieldValue(FLD_TRAN_APPROVER_LIMIT);
		}catch(error){
			nlapiLogExecution('error','GET APPROVER LIMIT',error.toString());
		}
